function [C, S] = QLiftDec3MaxMinMax(X, N)
%-----------------------------------------------------------------------------
%-----------------------------------------------------------------------------
% QLiftDec3MaxMinMax
...
%  Quincunx grid QBLACK is the union of the values at .10 and .01: "odd slots"
   ...
   QBLACKD111 = getcolor111(O) - hvLift111max(A101, A011, A110, cmin);                % Y1               % Y1
%  
...
%  Stage: predict
   DETAIL110 = QREDA110 - ...
       d1Lift110min(QREDA000, QREDA101, size(QREDA110, 2), cmax);        % Y2
   DETAIL011 = QREDA011 - ...
       d1Lift011min(QREDA000, QREDA101, size(QREDA011, 2), cmax);        % Y2
   clear QREDA110 QREDA011;
 
  [C, S] = stored1LiftDetail(DETAIL110, DETAIL011, lev+1, 'd', C, S);
%  Stage: update
   APPROX000 = QREDA000 + ...
     min(zeros(size(QREDA000),class(O)), ...
     d1Lift000min(DETAIL110, DETAIL011, size(QREDA000, 2), cmax));% X2
   APPROX101 = QREDA101 + ...
     min(zeros(size(QREDA101),class(O)), ...
     d1Lift101min(DETAIL110, DETAIL011, size(QREDA101, 2), cmax));% X2
% 
