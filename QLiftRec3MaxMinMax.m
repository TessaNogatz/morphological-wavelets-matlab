function X = QLiftRec3MaxMinMax(C, S)
%-----------------------------------------------------------------------------
% QLiftRec3MaxMinMax
...
%  Stage: reverse update 3
   sizeA000 = size(A000);
   A000 = A000 - max(zeros(sizeA000,tp),...
    d2Lift000max(D101, size(A000, 1), size(A000, 3), cmin));
   clear sizeA00;      
%
%  Stage: reverse predict 3
   A101 = D101 + ...
    d2Lift101max(A000, size(D101, 1), size(D101, 3), cmin);
%     
%  Merge
...
%  Stage: reverse update 2
   ...
   A000 = A000 - ...
         min(zeros(size(A000),tp), d1Lift000min(D110, D011, size(A000, 2), cmax));
   A101 = A101 - ...
         min(zeros(size(A101),tp), d1Lift101min(D110, D011, size(A101, 2), cmax));
% A000, A101
%  Stage: reverse predict 2 
   A110 = D110 + d1Lift110min(A000, A101, size(D110, 2), cmax);
   A011 = D011 + d1Lift011min(A000, A101, size(D011, 2) ,cmax);
   clear D110 D011;
%  
%  Level: Horizontal / Vertical Lift
%  Stage: reverse update 1
   [D010, D100, D001, D111] = retrievehvLift(lev-2, 'd', C, S);
   A000 = A000 - max(zeros(size(A000),tp),...
        hvLift000max(D010, D100, D001, cmin));     % X1
   A110 = A110 - max(zeros(size(A110),tp), ...
        hvLift110max(D100, D010, D111, cmin));     % X1
   A011 = A011 - max(zeros(size(A011),tp), ...
        hvLift011max(D001, D111, D010, cmin));     % X1
   A101 = A101 - max(zeros(size(A101),tp), ...
        hvLift101max(D111, D001, D100, cmin));     % X1
   
%
%  Stage: reverse predict 1 
   A010 = D010 + hvLift010max(A000, A110, A011, cmin);                % Y1
   A100 = D100 + hvLift100max(A110, A000, A101, cmin);                % Y1
   A001 = D001 + hvLift001max(A011, A000, A101, cmin);                % Y1
   A111 = D111 + hvLift111max(A101, A011, A110, cmin);
